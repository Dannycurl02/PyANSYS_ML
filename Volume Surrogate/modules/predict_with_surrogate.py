...
def visualize_prediction(predictions, coordinates, cold_vel, hot_vel, save_path=None):
    """
    Visualize predicted fields in 3D.
    ...
    step = max(1, n_cells // 3000)

    x = coordinates[::step, 0]
    y = coordinates[::step, 1]
    z = coordinates[::step, 2]
    ...
    temp_plot = predictions['temperature'][::step]
    scatter1 = ax1.scatter(x, y, z, c=temp_plot,
                           cmap='hot', s=10, alpha=0.6, edgecolors='none')
    ax1.set_xlabel('X (m)', fontsize=10)
    ax1.set_ylabel('Y (m)', fontsize=10)
    ax1.set_zlabel('Z (m)', fontsize=10)
    ax1.set_title(f'Predicted Temperature\n(Cold={cold_vel:.2f}, Hot={hot_vel:.2f} m/s)',
    ...
    press_plot = predictions['pressure'][::step]
    scatter2 = ax2.scatter(x, y, z, c=press_plot,
                           cmap='viridis', s=10, alpha=0.6, edgecolors='none')
    ax2.set_xlabel('X (m)', fontsize=10)
    ax2.set_ylabel('Y (m)', fontsize=10)
    ax2.set_zlabel('Z (m)', fontsize=10)
    ax2.set_title(f'Predicted Pressure\n(Cold={cold_vel:.2f}, Hot={hot_vel:.2f} m/s)',
                  fontsize=11, fontweight='bold')
...
def run_fluent_simulation(cold_vel, hot_vel, case_file, volume_name, iterations=200):
    """
    ...
    -------
    fluent_results : dict
        Dictionary with 3D volume field data from Fluent
    """
    ...
    # Extract field data from 3D volume
    print(f"  Extracting field data from 3D volume '{volume_name}'...")
    ...
    velocity_x = np.array(vx_dict[volume_name])
    ...
    velocity_y = np.array(vy_dict[volume_name])
    ...
    velocity_z = np.array(vz_dict[volume_name])
    ...
    coordinates = coords_flat.reshape((n_cells, 3))
...
def create_comparison_plot(fluent_results, predictions, coordinates, cold_vel, hot_vel, save_path=None):
    """
    Create 3x3 comparison plot in 3D: Fluent | Prediction | Error.
    ...
    for row, (field, label, cmap) in enumerate(zip(fields, labels, cmaps)):
        ...
        vmin = min(fluent_plot.min(), pred_plot.min())
        vmax = max(fluent_plot.max(), pred_plot.max())

        # Column 1: Fluent (Ground Truth)
        ax1 = fig.add_subplot(3, 3, row*3 + 1, projection='3d')
        scatter1 = ax1.scatter(x, y, z, c=fluent_plot, cmap=cmap, s=8, alpha=0.6,
        ...
        ax1.set_title(f'Fluent - {field.replace("_", " ").title()}', fontsize=10, fontweight='bold')
        cbar1 = plt.colorbar(scatter1, ax=ax1, pad=0.1, shrink=0.7)
        cbar1.set_label(label, fontsize=8)

        # Column 2: Prediction
        ax2 = fig.add_subplot(3, 3, row*3 + 2, projection='3d')
        scatter2 = ax2.scatter(x, y, z, c=pred_plot, cmap=cmap, s=8, alpha=0.6,
                               edgecolors='none', vmin=vmin, vmax=vmax)
        ...
        cbar2.set_label(label, fontsize=8)

        # Column 3: Error (centered at zero)
        ax3 = fig.add_subplot(3, 3, row*3 + 3, projection='3d')
        ...
        scatter3 = ax3.scatter(x, y, z, c=error_plot, cmap='RdBu_r', s=8, alpha=0.6,
        ...
        cbar3.set_label(f'Error ({label.split("(")[1].split(")")[0]})', fontsize=8)

        # Set equal aspect ratio for all 3D plots in this row
        for ax in [ax1, ax2, ax3]:
            max_range = np.array([x.max()-x.min(), y.max()-y.min(), z.max()-z.min()]).max() / 2.0
            mid_x = (x.max()+x.min()) * 0.5
            mid_y = (y.max()+y.min()) * 0.5
...
if __name__ == "__main__":
    ...
    for i, model_dir in enumerate(model_dirs, 1):
        # Get dataset file in this model folder (exclude surrogate model files)
        dataset_files = [f for f in model_dir.glob("*.npz") if not f.name.startswith("surrogate_")]
    ...
    # Find dataset in model directory (exclude surrogate model files)
    dataset_files = [f for f in MODEL_DIR.glob("*.npz") if not f.name.startswith("surrogate_")]
    if not dataset_files:
        print(f"\n✗ No dataset found in model directory: {MODEL_DIR}")
        print(f"  Looking for files not starting with 'surrogate_'")
        exit(1)
