...
def run_doe(cold_vel_array, hot_vel_array, case_file, plane_name,
    ...
    # Solution variable data accessor (for 3D volume extraction)
    sv_data = solver_session.fields.solution_variable_data
    ...
    try:
        for i, cold_vel in enumerate(cold_vel_array):
            for j, hot_vel in enumerate(hot_vel_array):
                ...
                # Extract volume field data (using solution variables)
                print(f"  Extracting volume data from '{plane_name}'...")

                try:
                    # Get temperature (SV_T)
                    temp_dict = sv_data.get_data(
                        variable_name="SV_T",
                        zone_names=[plane_name],
                        domain_name="mixture"
                    )
                    temp_data = np.array(temp_dict[plane_name])
                    ...
                    press_data = np.array(press_dict[plane_name])

                    # Get velocity components (SV_U, SV_V, SV_W)
                    vx_dict = sv_data.get_data(
                        variable_name="SV_U",
                        zone_names=[plane_name],
                        domain_name="mixture"
                    )
                    vx_data = np.array(vx_dict[plane_name])
                    ...
                    vy_data = np.array(vy_dict[plane_name])
                    ...
                    vz_data = np.array(vz_dict[plane_name])
                    ...
                    if coordinates_data is None:
                        ...
                        coords_flat = np.array(coord_dict[plane_name])
                        # Reshape from flat [x1,y1,z1,x2,y2,z2,...] to (n_cells, 3)
                        n_cells = len(temp_data)
                        coords = coords_flat.reshape((n_cells, 3))
                        x_coords = coords[:, 0]
                        y_coords = coords[:, 1]
                        z_coords = coords[:, 2]

                        # Stack into (n_cells, 3) array
