...
class DirectNN(nn.Module):
    ...
    def _calculate_hidden_dims(self, input_dim, output_dim):
        ...
        Strategy: Conservative architecture to prevent overfitting on small datasets.
        Uses geometric mean between input and output as basis.
        """
        # Geometric mean gives good starting point
        geometric_mean = int(np.sqrt(input_dim * output_dim))

        # Cap based on output dimension to prevent over-parameterization
        if output_dim < 500:
            # Small problem: shallow network
            h1 = min(128, geometric_mean * 2)
            h2 = min(256, geometric_mean * 4)
            return [h1, h2, h1]
        elif output_dim < 2000:
            # Medium problem
            h1 = min(256, geometric_mean * 2)
            h2 = min(512, geometric_mean * 4)
            h3 = min(768, geometric_mean * 6)
            return [h1, h2, h3, h2, h1]
        elif output_dim < 5000:
            # Large problem
            h1 = min(256, geometric_mean)
            h2 = min(512, geometric_mean * 2)
            h3 = min(1024, geometric_mean * 3)
            return [h1, h2, h3, h2, h1]
        else:
            # Very large problem
            h1 = min(512, geometric_mean)
            h2 = min(1024, geometric_mean * 2)
            h3 = min(2048, geometric_mean * 3)
            return [h1, h2, h3, h2, h1]
...
class DirectNNModel:
    ...
    def train(self, X_train, Y_train, validation_split=0.2, early_stopping_patience=50):
        ...
        # Create data loaders
        # Ensure batch size is reasonable for dataset size
        effective_batch_size = min(self.config['batch_size'], max(4, len(X_train_split) // 4))
